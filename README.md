<!-- Header -->
<div align="center"> <img src="https://capsule-render.vercel.app/api?type=waving&color=timeGradient&height=250&section=header&text=Henrique%20Monteiro%20Cardoso&fontSize=50&fontAlignY=35&desc=Full%20Stack%20Developer&descAlignY=55&descAlign=50" alt="Banner" />

<div align="center">
  <br>
  <a href="https://www.linkedin.com/in/henrique-monteiro-cardoso-ba3716229/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"></a>
  <a href="mailto:henriquemon17@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"></a>
  <a href="https://wa.me/5515988027261" target="_blank"><img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"></a>
  <a href="https://discordapp.com/users/.crazywen" target="_blank"><img src="https://img.shields.io/badge/Discord-5865F2?style=for-the-badge&logo=discord&logoColor=white"></a>
  <br><br>
</div>

---

## 👨‍💻 Sobre Mim

Engenheiro de Software Fullstack com sólida experiência na concepção, arquitetura e desenvolvimento de soluções tecnológicas de ponta a ponta. Focado em **escalabilidade, performance e qualidade de código**, aliando fortes práticas de engenharia a uma visão de produto centrada no usuário.

Minha especialidade reside em resolver problemas complexos por meio de arquiteturas resilientes (microsserviços, event-driven, cloud-native) e interfaces de usuário de altíssimo nível.

- ⚙️ **Arquitetura & Backend:** Especialista em construção de APIs RESTful estruturadas, modelagem de bancos de dados relacionais e NoSQL, aplicadas em esteiras CI/CD eficientes. (Node.js, Java, Python)
- 🖥️ **Frontend & UX:** Desenvolvimento de interfaces modernas, performáticas e acessíveis, adotando tecnologias contemporâneas e componentização avançada. (React, Next.js, Angular, TypeScript)
- ☁️ **Cloud & Infraestrutura:** Orquestração de serviços e deploy de aplicações em ambientes de alta disponibilidade e escabilidade.
- 🚀 **Cultura & Liderança Técnica:** Defensor de Clean Code, TDD, DDD e metodologias ágeis. Comprometido com a disseminação de conhecimento e padronização arquitetural na comunidade técnica.

---

## 🛠️ Stack Tecnológica & Ecossistema

<div align="center">
  <table>
    <tr>
      <td align="center" width="25%"><b>💻 Front-end & UI</b></td>
      <td align="center" width="25%"><b>⚙️ Back-end & APIs</b></td>
      <td align="center" width="25%"><b>🗄️ Database & Cloud</b></td>
      <td align="center" width="25%"><b>🛠️ Infra & Ferramentas</b></td>
    </tr>
    <tr>
      <td align="center"><img src="https://skillicons.dev/icons?i=ts,react,nextjs,angular,tailwind,html,css" height="40"/><br></td>
      <td align="center"><img src="https://skillicons.dev/icons?i=java,py,cpp,nodejs,php" height="40"/><br></td>
      <td align="center"><img src="https://skillicons.dev/icons?i=mysql,postgres,firebase,gcp" height="40"/><br></td>
      <td align="center"><img src="https://skillicons.dev/icons?i=git,github,docker,vscode,figma,arduino" height="40"/><br></td>
    </tr>
  </table>
</div>

---

## 🚀 Projetos em Destaque

<table align="center">
  <tr>
    <td align="center" width="50%">
      <a href="https://v0-saas-landing-page-bi83mscdt.vercel.app/" target="_blank">
        <img src="https://img.shields.io/badge/Safe%20Finance-00F7E0?style=for-the-badge&logo=react&logoColor=black" />
      </a><br/>
      <b>Plataforma de Gestão Financeira (SaaS)</b><br/>
      <i>Arquitetura front-end escalável, performance otimizada (Core Web Vitals) e UI/UX premium.</i><br/>
      <br/>
      <img src="https://skillicons.dev/icons?i=react,ts,tailwind,vercel" height="25"/>
    </td>
    <td align="center" width="50%">
      <a href="https://v0-portfolio-website-zeta-plum.vercel.app/" target="_blank">
        <img src="https://img.shields.io/badge/Personal%20Portfolio-007396?style=for-the-badge&logo=next.js&logoColor=white" />
      </a><br/>
      <b>Apresentação Profissional & Engine de Rendering</b><br/>
      <i>Ecossistema moderno utilizando SSR/SSG, Design System customizado e deploy automatizado.</i><br/>
      <br/>
      <img src="https://skillicons.dev/icons?i=nextjs,react,ts,tailwind" height="25"/>
    </td>
  </tr>
  <tr>
    <td align="center" width="50%">
      <a href="https://github.com/HenriqueMC17/JogoDaVelha" target="_blank">
        <img src="https://img.shields.io/badge/Tic%20Tac%20Toe%20Engine-ff5722?style=for-the-badge&logo=javascript&logoColor=white" />
      </a><br/>
      <b>Lógica de Jogo Web-Based</b><br/>
      <i>Implementação de algoritmos de estado interativo, gestão de state global e DOM manipulation pura.</i><br/>
      <br/>
      <img src="https://skillicons.dev/icons?i=js,html,css" height="25"/>
    </td>
    <td align="center" width="50%">
      <a href="https://github.com/HenriqueMC17/OrganizadorDeArquivos" target="_blank">
        <img src="https://img.shields.io/badge/File%20System%20Organizer-306998?style=for-the-badge&logo=python&logoColor=white" />
      </a><br/>
      <b>Ferramenta de Automação de Infraestrutura</b><br/>
      <i>Scripting inteligente em Python para gerenciamento I/O eficiente e padronização de diretórios.</i><br/>
      <br/>
      <img src="https://skillicons.dev/icons?i=py" height="25"/>
    </td>
  </tr>
</table>

---

## 📈 Engineering Analytics

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=HenriqueMC17&show_icons=true&theme=tokyonight&count_private=true&hide_border=true&border_radius=10" height="195" alt="GitHub Stats"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=HenriqueMC17&layout=compact&theme=tokyonight&langs_count=12&custom_title=Most%20Used%20Languages&hide_border=true&border_radius=10" height="195" alt="Top Languages"/>
</div>
<br>
<div align="center">
  <img src="https://streak-stats.demolab.com?user=HenriqueMC17&theme=tokyonight&hide_border=true&date_format=M%20j%5B%2C%20Y%5D&mode=weekly&border_radius=10" height="190" alt="Streak Stats"/>
</div>
<br>
<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=HenriqueMC17&radius=10&theme=tokyonight&area=true&hide_border=true&custom_title=Git+Activity+Telemetry" width="100%" alt="Gráfico de Atividades"/>
</div>
<br>
<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/HenriqueMC17/HenriqueMC17/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/HenriqueMC17/HenriqueMC17/output/github-contribution-grid-snake.svg">
    <img alt="github contribution grid snake animation" src="https://raw.githubusercontent.com/HenriqueMC17/HenriqueMC17/output/github-contribution-grid-snake.svg">
  </picture>
</div>

---

## 🎧 Focus & Productivity

<div align="center">
  <a href="https://open.spotify.com/user/315p554yeqawff6qlol2mtdc7tfm?si=f162ae17e24044ad" target="_blank">
    <img src="https://img.shields.io/badge/Spotify-1DB954?style=for-the-badge&logo=spotify&logoColor=white" alt="Spotify Profile" />
  </a>
  <br><br>
  <a href="https://open.spotify.com/user/315p554yeqawff6qlol2mtdc7tfm?si=f162ae17e24044ad" target="_blank">
    <img src="https://spotify-recently-played-readme.vercel.app/api?user=315p554yeqawff6qlol2mtdc7tfm&count=3&unique=true" alt="Spotify Recently Played" />
  </a>
</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=timeGradient&height=100&section=footer" width="100%"/> </div>
